...
from pairag.integrations.data_analysis.text2sql.utils.sql_utils import (
    DefaultSQLRetriever,
    SQLParserMode,
    BaseSQLParser,
    DefaultSQLParser,
)
from pairag.integrations.data_analysis.text2sql.utils.info_utils import (
    get_schema_desc,
)
from pairag.integrations.data_analysis.text2sql.utils.prompts import (
...
class SQLNodeGenerator(SQLGenerator):
    """
    基于自然语言问题和db_info等生成候选的sql查询语句，
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        super().__init__(sql_database, llm)
        self._sql_database = sql_database
        self._dialect = self._sql_database.dialect
        self._tables = list(sql_database._usable_tables)
        self._sql_retriever = DefaultSQLRetriever(sql_database, return_raw=return_raw)
        self._llm = llm or Settings.llm
