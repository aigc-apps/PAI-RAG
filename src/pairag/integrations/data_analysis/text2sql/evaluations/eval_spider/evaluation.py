...
from pairag.integrations.data_analysis.text2sql.evaluations.eval_spider.process_sql import (
    get_schema,
    Schema,
    get_sql,
)
from pairag.integrations.data_analysis.text2sql.evaluations.eval_spider.exec_eval import (
    eval_exec_match,
)
...
def print_formated_s(row_name, target, element_format):
    template = "{:20} " + " ".join([element_format] * len(target))
    print(template.format(row_name, *target))
