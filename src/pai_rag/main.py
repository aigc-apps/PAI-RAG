...
def ui(host, port, rag_url):
    print(f"starting the filebrowser of /app with subpath /filebrowser on {8012}")
    subprocess.Popen(f"rm -rf /tmp/filebrowser.db && /bin/filebrowser -b /filebrowser --address 0.0.0.0 -p {8012} -r /app/localdata --noauth -d /tmp/filebrowser.db",shell=True)
