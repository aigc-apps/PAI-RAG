...
- 如果用户想要查询某一个领域的新闻，首先判断该领域是否在给定的领域列表 {domain_list} 中，若在，请直接生成list_news意图并带上相应领域的名字，生成结果格式为 JSON 对象：{{ "intent": "list_news", "news_topics": ["domain_name"] }}。若不在，请生成chat_news意图，生成结果格式为 JSON 对象：{{ "intent": "chat_news", "query": "query" }}。例如，查询“科技的新闻”，首先“科技”是一个领域，并且该领域存在于给定的列表 [\'科技\', \'娱乐\', \'社会\', \'体育\'] 中，因此生成结果应该为{{ "intent": "list_news", "news_topics": ["科技"] }}。查询“医疗的新闻”，首先“医疗”是一个领域，但是“医疗”不在给定的列表[\'科技\', \'娱乐\', \'社会\', \'体育\'] 中，因此生成结果应该为{{ "intent": "chat_news", "query": "医疗的新闻" }}。
- 如果用户想要了解或者想要查询某个具体的新闻，你会根据上下文信息对用户的查询进行改写，并生成chat_news意图，改写之后的意图和查询格式为 JSON 对象：{{ "intent": "chat_news", "query": "new query" }}。例如，查询“陈奕迅的新闻”，这是一条具体的新闻查询，因此生成结果为{{ "intent": "chat_news", "query": "陈奕迅的新闻" }}。
