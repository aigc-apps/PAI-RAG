...
class FileDeltaDatasource(FileBasedDatasource):
    ...
    def get_read_tasks(self, parallelism: int) -> List[ReadTask]:
        ...
        def read_files() -> List[Dict]:
            ...
            file_results = [
                pyarrow_table_from_pydict(value_dict) for value_dict in file_info_list
            ]

            logger.info(
                f"Loaded {len(file_results)} files from {self.config.input_path}. Enable delta: {self.config.enable_delta}"
            )
            return file_results

        return [
            ReadTask(read_files, metadata=BlockMetadata(None, None, None, None, None))
        ]
