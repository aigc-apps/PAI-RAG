...
class DBDescriptor:
    ...
    def __init__(
        ...
        db_summary_prompt: Optional[PromptTemplate] = None,
    ):
    ...
    async def asummarize_schema(self) -> str:
...
class AnalysisOutput(BaseModel):
    ...
    table_column_analysis: List[str] = Field(
        description="基于提供的数据表结构信息和样本信息, 提供每个字段的补充描述(专业术语解释、时间格式解释等)和2条数据样本, 具体格式为: 表名|字段名|字段类型|字段描述|2条数据样本, \n要求尽量简单明了, 禁止生成提供信息以外的内容"
    )
