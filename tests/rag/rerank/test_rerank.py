...
class TestDashscopeReranker:
    ...
    @pytest.mark.skipif(
        ...
        reason="需要设置有效的 DASHSCOPE_API_KEY 环境变量"
    )
    async def test_rerank_effectiveness(self, dashscope_reranker, sample_query, sample_documents):
...
class TestOpenAICompatibleReranker:
    ...
    @pytest.mark.asyncio
    @pytest.mark.skipif(
        not (os.getenv("OPENAI_RERANK_BASE_URL") or os.getenv("OPENAI_RERANK_API_KEY") or os.getenv("OPENAI_RERANK_BASE_MODEL")),
        reason="需要设置有效的 OPENAI_RERANK_BASE_URL, OPENAI_RERANK_API_KEY, OPENAI_RERANK_MODEL 环境变量"
    )
    async def test_rerank_effectiveness(self, openai_reranker, sample_query, sample_documents):
