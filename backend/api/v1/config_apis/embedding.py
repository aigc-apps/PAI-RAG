...
async def create_embedding(
    ...
    try:
        embedding = await embedding_service.create_embedding(embedding_create)
...
async def get_embeddings(
    ...
    try:
        if not model_name:
            embedding_models = await embedding_service.list_embeddings(page, size)
            return success_response(
                data=embedding_models,
                message="查询embedding模型列表成功"
            )
        else:
            embedding_model = await embedding_service.get_embedding_by_model_name(model_name)
...
async def update_embedding(
    ...
    try:
        embedding_model = await embedding_service.update_embedding(emb_id, new_embedding)
...
async def delete_embedding(
    ...
    try:
        await embedding_service.delete_embedding(emb_id)
