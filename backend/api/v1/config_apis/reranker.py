...
async def create_reranker(
    ...
    try:
        reranker = await reranker_service.create_reranker(reranker_create=reranker_create, tenant_id=tenant_id)
...
async def get_rerankers(
    ...
    try:
        if not model_name:
            reranker_models = await reranker_service.list_rerankers(tenant_id=tenant_id, page=page, size=size)
