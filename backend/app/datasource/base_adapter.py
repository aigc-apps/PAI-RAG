...
Ported verbatim from backend/rag/datasource/base_adapter.py.
"""
...
from app.datasource.schema import DiscoveredDoc, SourceDocument
...
class BaseAdapter(ABC):
    ...
    def __init__(self, datasource_key: str, source_config: Optional[dict] = None):
        ...
        # crawl page failed transiently). The sync loop then SKIPS deletions so a
    ...
    def validate_config(self) -> None:
        """Raise ValueError if ``source_config`` is missing required fields.

        Called at create/update time (before any network access) so a
        misconfigured source is rejected up front. Default: no required fields.
        """
        return None
