...
class AgentKnowledgeConfig(BaseModel):
    ...
    can reach — but each id is still permission-checked per request, so an agent
...
class AgentProfile(BaseModel):
    ...
    # read-only /opt/code layer. Injected into the (cacheable) system prompt when
    ...
    providers=[
        ...
        ProviderConfig(
            ...
            settings={
                ...
                # Read-only code layer baked into the sandbox image at /opt/code
                # (repos are subdirs). Turn on for a template that ships the layer;
                # exposes AGENT_CODE_PATH + the code-exploration system prompt.
                "code_layer_enabled": False,
...
def apply_runtime_status(doc: AgentConfigDocument, settings, router) -> AgentConfigDocument:
    ...
    if llm is not None:
        ...
        # A blank agent.model means "inherit the deployment default". Leave it
        # blank: the resolved default is exposed via llm.settings["default_model"]
        # for the UI to render as an "inherits" hint. Back-filling it here would
        # round-trip through PUT /v1/config and silently freeze each agent to the
        # current default, defeating the inherit affordance.
