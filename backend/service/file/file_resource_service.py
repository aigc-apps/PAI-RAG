...
class FileResourceService:
    ...
    async def create_from_upload(
        ...
        *,
        ...
        Returns (entity, is_new). If a tenant+md5+purpose duplicate exists, returns
        the existing row and skips the OSS upload (is_new=False).
        """
        ...
        # 2. Create row (id generated by model default_factory), then upload to
        #    a path keyed by the new id so later reads are deterministic.
        entity = FileEntity(
            tenant_id=tenant_id,
            ...
            mime_type=get_file_mime_type(preview.file_extension),
            status=FileStatus.pending.value,
            file_metadata=metadata or {},
            alias_id=alias_id,
            expires_at=_compute_expires_at(purpose, expires_in_seconds),
        )
        ...
        try:
            ...
            # Race with a concurrent dedup-miss: re-query and return the winner.
            await self.session.rollback()
            winner = await self.find_by_md5(
                md5=stored.file_md5, purpose=purpose, tenant_id=tenant_id
            )
            if winner:
                return winner, False
            raise
