...
class McpserverService:
    ...
    async def get_mcpserver(self, mcp_id: str) -> Optional[McpServerEntity]:
        """
        ...
        """
        return await self.session.get(McpServerEntity, mcp_id)

    async def get_mcpserver_by_name(self, name: str) -> Optional[McpServerEntity]:
        """
        ...
        Returns:
            McpServerEntity if found, None otherwise
        """
        statement = select(McpServerEntity).where(McpServerEntity.name == name)
        result = await self.session.exec(statement)
        return result.first()

    async def get_mcpserver_by_ids(self, ids: List[str]) -> List[McpServerEntity]:
        """
        ...
        """
        statement = select(McpServerEntity).where(McpServerEntity.id.in_(ids))
        result = await self.session.exec(statement)
    ...
    async def list_mcpservers(
        ...
        """
        # Build base query
        base_query = select(McpServerEntity)

        # Add name filter if provided
        if name is not None:
            base_query = base_query.where(McpServerEntity.name == name)
    ...
    async def create_mcpserver(self, mcp_data: McpServerCreate) -> McpServerEntity:
        """
    ...
    async def update_mcpserver(
        self, mcp_id: str, update_data: McpServerCreate
    ) -> McpServerEntity:
        """
        ...
        """
        mcp = await self.session.get(McpServerEntity, mcp_id)
        if not mcp:
    ...
    async def delete_mcpserver(self, mcp_id: str) -> None:
        """
    ...
    async def get_all_mcpservers(self) -> List[McpServerEntity]:
        """
        ...
        """
        statement = select(McpServerEntity)
