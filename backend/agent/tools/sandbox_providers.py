...
def _build_nas_config(provider: "AgentRunRestSandboxProvider", scope, scope_key: str) -> Optional[Dict[str, Any]]:
    ...
    /mnt/skills/<id> and a per-user read-write mount at /mnt/user. All
    mountPoints share userId/groupId (default 1000). Returns None when no
    mountPoints apply so the field is omitted from the create payload. (The
    read-only code layer at /opt/code is baked into the image, not NAS-mounted,
    ...
    # The read-only code layer at /opt/code is baked into the sandbox image, not
    # NAS-mounted, so it contributes no mountPoint here.
    if not mount_points:
        return None
    return {
