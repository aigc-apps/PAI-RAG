"""SQLite-backed concrete implementation of `BaseSessionStore`.

The class lives here so the abstract interface in `backend.session_store_base`
stays free of SQLite-specific concerns (PRAGMAs, journal modes, the
`@_writer` decorator, FUSE detection). Add new backends as sibling modules
(e.g. `session_store_postgres.py`) — call sites import only from `session_store`
or `backend.session_store_base`, never from this file directly.
"""
import functools
import json
import os
...
# Reentrant so a write method may invoke another decorated helper without
# deadlocking. Only serializes writers within one Python process; cross-process
# contention is handled by SQLite's busy_timeout + the retry loop in `_writer`.
...
def _writer(method):
    """Decorator: serialize writers within this process + retry on SQLite lock errors."""
            ...
            with _PROCESS_WRITER_LOCK:
                try:
                    ...
                    if not _is_lock_error(e):
                        raise
                    print(
                        f'[SessionStore] {method.__name__} hit lock; retrying in {delay*1000:.0f}ms',
                        file=sys.stderr,
                    )
            time.sleep(delay)
        with _PROCESS_WRITER_LOCK:
...
def sqlite_journal_mode():
    ...
    if mode not in SQLITE_JOURNAL_MODES:
        mode = 'DELETE'
    # WAL needs mmap-backed shared memory in *-shm; OSS-FUSE / FUSE doesn't
    # support that. Set MEMORY_BACKEND_FS=ossfuse in such deployments.
    if mode == 'WAL' and os.environ.get('MEMORY_BACKEND_FS', '').lower() in ('ossfuse', 'fuse'):
        return 'DELETE'
    return mode
...
class SQLiteSessionStore(BaseSessionStore):
    ...
    def set_run_last_event(self, run_id, user_id, event_id):
        # Layer 1: cursor moved to Redis (`RedisBus.set_run_cursor`) to remove
        # the per-poll SQLite write. Kept as a no-op for one release so any
        # surviving callers don't break; remove once all sites are migrated.
        return
    ...
    def _connect(self):
        ...
        # PRAGMAs are per-connection. Setting them only in _init_db doesn't
        # apply to subsequent connections, which is why under load we'd see
        # "database is locked" without the busy_timeout kicking in.
