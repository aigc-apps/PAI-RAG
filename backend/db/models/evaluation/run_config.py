...
class EvaluatorConfig(SQLModel):
    name: str = Field(default="ExactMatch")
    model_id: str = Field(default="")
    case_sensitive: bool = Field(default=False)
    ignore_punctuation: bool = Field(default=False)

class EvalRunConfigCreate(SQLModel):
...
class EvalRunConfigEntity(EvalRunConfigCreate, table=True):
    __tablename__ = "pai_evaluation_run_config"
    ...
    evaluator_config: dict = Field(default_factory=dict, sa_column=Column(JSON))
