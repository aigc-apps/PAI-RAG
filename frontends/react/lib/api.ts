import type { AgentUpdate, AuthResponse, RunCreateResponse, RunStreamEnvelope, SessionDetail, SessionSummary, StreamEvent, UserProfile } from "@/lib/types";
...
export async function stopRun(runId: string): Promise<void> {
  const response = await fetch(`/api/runs/${runId}/stop`, { method: "POST", headers: authHeaders() });
  await assertOk(response);
}
