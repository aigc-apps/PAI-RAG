...
async def main():
    ...
    messages = [
        {
            "role": "user",
            "content": "List 3 effective ways to learn programming.",
        }
    ]
...
# Initialize model
model = get_llm_model(
    llm_provider="openai", model_name="gpt-4o", api_key="your_api_key"
)
...
class CustomProvider(LLMProviderBase):
    def _init_provider(self):
        ...
        return {"api_key": self.api_key, "endpoint": self.base_url}
...
model = get_llm_model(
    llm_provider="custom_provider", model_name="custom-model"
)
```
