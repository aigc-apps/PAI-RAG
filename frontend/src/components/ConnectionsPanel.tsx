...
import { Plus, Trash2, Zap, Loader2, CircleCheck, CircleAlert, X } from "lucide-react";
...
import { BTN_GHOST, BTN_PRIMARY, CARD } from "../lib/ui";
...
function CreateButton({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="inline-flex h-10 items-center justify-center gap-2 rounded-[var(--radius)] border border-[var(--border-strong)] bg-[var(--surface)] px-4 text-sm font-semibold text-[var(--text)] shadow-[0_2px_10px_rgba(15,23,42,0.09)] transition-[background-color,border-color,box-shadow,transform] hover:-translate-y-px hover:border-[var(--text-muted)] hover:bg-[var(--bg-elevated)] hover:shadow-[0_6px_16px_rgba(15,23,42,0.11)] focus-ring max-sm:w-full"
    >
      <span className="grid h-5 w-5 place-items-center rounded-[var(--radius-sm)] border border-[var(--border)] bg-[var(--bg-elevated)]">
...
function FormDialog({
  ...
  return (
    ...
    >
      <div className="flex max-h-[85vh] w-full max-w-2xl flex-col rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--bg-elevated)] shadow-xl">
...
function ConnectionsSection({
  ...
  const submit = () => {
    ...
    void onSave({ ...cat, providers: list }, closeDialog);
  };
  ...
  return (
    <section className={cn(CARD, "space-y-4 p-[18px]")}>
      ...
      {dialogOpen && (
        <FormDialog
          title={editing ? t("conn.editConnection", { name: editing }) : t("conn.addConnection")}
          onClose={closeDialog}
        >
...
function ModelsSection({
  ...
  return (
    <section className={cn(CARD, "space-y-4 p-[18px]")}>
      ...
      {dialogOpen && (
        <FormDialog
          title={editing ? t("conn.editModel", { ref: `${editing.provider}/${editing.id}` }) : t("conn.registerModel")}
          onClose={closeDialog}
        >
          <div className="grid gap-3">
