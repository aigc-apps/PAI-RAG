...
class TxtGenerator:
    ...
    def get_qa_text(
        ...
        """
        输入原始文档和约束规则，拼接得到完整prompt，输入qa提取llm，得到llm提取结果

        Args:
            prompt (str)

        Returns:
            qa_text (str): qa提取llm的回复结果
        """
        qa_text = self.llm(prompt)
        return qa_text

    def extract_qa_dict(self, qa_text):
        """
        从qa提取llm的回复中提取出qa对
        ...
        pairs = qa_text.split("Human: ")
        qa_dict = {}

        for pair in pairs:
            if len(pair)>0:
                qa = pair.split("Assistant: ")
                if len(qa) == 2 and isValidQA(qa):
                    q, a = qa[0].strip('\n'), qa[1].strip('\n')
                    qa_dict.update({q: a})
        
        return qa_dict

    def generate_qa(self, text_content):
        """
        输入原始文档，提取出qa对字典

        Args:
            text_content (str): 原始文档

        Returns:
            summary_dict (dict): qa对字典
            {Q1: A1, Q2: A2}
        """
        prompt = self.get_prompt(text_content)
        qa_text = self.get_qa_text(prompt)
        print(f"[INFO] qa text generated by llm:\n{qa_text}")
        qa_dict = self.extract_qa_dict(qa_text)
        return qa_dict
...
class HtmlGenerator:
    ...
    def get_qa_text(
        ...
        """
        输入html源码和约束规则，拼接得到完整prompt，输入qa提取llm，得到llm提取结果

        Args:
            input_text (str): html源码
            prompt_prefix (str): prompt前缀
            prompt_tail (str): prompt后缀

        Returns:
            collected_message (str): qa提取llm的回复结果
        """
    ...
    def generate_qa(
        ...
        Returns:
            ...
            {Q1: A1, Q2: A2}
        """
        prompt_prefix, prompt_tail = self.get_prompt(src_text)
        ...
        while try_cnt <= try_lim:
            try:
                QA_text = self.get_qa_text(src_text, prompt_prefix, prompt_tail)
                QA_dict = self.extract_qa_dict(QA_text)
                if QA_dict:
