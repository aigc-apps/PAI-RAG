...
- curl 请求示例：
  ...
  ```bash
  curl -X 'POST' '{EAS_SERVICE_URL}/api/v1/query' -H 'Authorization: EAS_TOKEN' -d '{ "messages": [{"role": "user","content": "What are the first programs the author write?"}],"stream": false, "index_name": "default", "return_reference": true } '
  ```
