...
POST /api/v1/query/llm

### 知识库检索

POST /api/v1/query/retrieval
...
POST /api/v1/query/data_analysis
...
- 请求参数：
  ...
  - enable_raptor: 是否开启raptor(默认为false)
  - enable_multimodal:是否开启多模态(默认为false)
...
- curl 请求示例：
  ...
  ```bash
  curl -X 'POST' http://localhost:8680/api/v1/upload_data -H 'Authorization: EAS_TOKEN' -H 'Content-Type: multipart/form-data' -F 'files=@example_data/paul_graham/paul_graham_essay.txt'
  ```
