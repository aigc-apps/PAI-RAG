...
class GaiaAgentRunner:
    ...
    def _judge_answer(self, data_item: dict, result: Output):
        ...
        result_file = os.path.join(
            os.path.join(os.path.dirname(os.path.abspath(__file__)), "output"), "results.jsonl"
        )
        if match:
...
if __name__ == "__main__":
    ...
    def send_output(output, output_file):
    ...
    async def main():
        ...
        try:
            with open(args.testfile, "r", encoding="utf-8") as infile:
                for line in infile:
                    # 解析每一行 JSON 数据
                    record = json.loads(line.strip())
                    prompt = json.dumps({"task_id": record.get("task_id")})
                    output_file = os.path.join(
                        output_dir, f"output_{record.get("task_id")}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
                    )
                    
                    async for i in agent_runner.run(prompt):
                        send_output(i, output_file)
        except Exception as e:
            logger.error(
